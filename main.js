var version = '0.2 | Sexiness Update';
var startUpMsg = 'Welcome to NCS version ' + version + '!';
var newFeaturesMsg = "<a href='https://electricgaming.ga/ncs/' target='_blank'>The NCS Website is now online!</a><br><b><i>MAJOR SMEXINESS INBOUND</b></i>(™)<br>Ok it's definitely not that great but still... not too bad, right?";
var uname;
var lastSelected;
...
function NCSinit(){
  ...
  $('#app-right').append('<div id="NCS-menu" style="display:none;opacity:0.4;background-color:whitesmoke;box-shadow: inset 0px 0px 10px 4px rgba(0,0,0,0.9);"></div>');
  ...
  $('#NCS-f1,#NCS-f2,#NCS-f3,#NCS-f4').on('click',NCSfeatures);
  $('head').append('<style type="text/css">#NCS-btn:hover{cursor:pointer;background-color:grey;}.NCS-checkmark{float:right;background-image:url("http://i.imgur.com/VzMmfJJ.png?1");background-repeat:no-repeat;height:15px;width:15px;margin-right:25px;}.NCSf{height:15px;word-wrap:break-word;opacity:0.8;padding-top:13px;padding-bottom:13px;padding-left:15px;color:black;}.NCSf:hover{cursor:pointer;box-shadow:inset 0px 0px 9px 1px rgba(139,0,139,0.8);}</style>');
